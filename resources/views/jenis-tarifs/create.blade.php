...
@section('title', 'Tambah Jenis Tarif')

@section('content')
<div class="max-w-3xl mx-auto">
    <x-card>
        <x-slot name="title">Tambah Jenis Tarif Baru</x-slot>
        <x-slot name="subtitle">Lengkapi data jenis tarif di bawah ini</x-slot>
        <x-slot name="actions">
        ...
        <form action="{{ route('jenis-tarifs.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-input name="code" label="Kode" placeholder="Contoh: laboratorium" :required="true" />
                ...
                <div class="md:col-span-2">
                    <x-input name="name" label="Nama" placeholder="Contoh: Laboratorium" :required="true" />
                </div>
                ...
                <div class="md:col-span-2 flex justify-end gap-2 pt-2">
                    ...
                    <button type="submit" class="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold text-white rounded-md bg-sp-primary hover:bg-sp-primary-dark transition-colors">
                        <i class="bi bi-check-lg"></i> Simpan Jenis Tarif
                    </button>
                </div>
            </div>
        </form>
