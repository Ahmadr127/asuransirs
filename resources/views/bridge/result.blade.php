    ...
    <x-card padding="false">
        ...
        <x-slot name="actions">
            @if(($result['summary']['matched'] ?? 0) > 0 || ($result['summary']['suggested'] ?? 0) > 0)
                <form action="{{ route('bridge.generate') }}" method="POST" onsubmit="return confirm('Generate Excel baru? SERVICECODE berubah untuk row MATCHED + row AMBIGUOUS/NOT_FOUND yang ada sarannya; SERVICECODE KELAS mengikuti master bila ditemukan.');">
                    @csrf
