...
<div class="space-y-6">
            ...
            <div class="flex justify-between items-start">
                <div>
                    <div class="flex items-center gap-3 mb-2">
                        <span class="inline-flex px-2 py-1 text-xs font-mono bg-gray-200 text-gray-700 rounded">{{ $serviceClass->code }}</span>
                        @if($serviceClass->status === 'active')
                    ...
                    <h2 class="text-2xl font-bold text-gray-900">{{ $serviceClass->name }}</h2>
                    <p class="text-sm text-gray-500 mt-1">Dibuat {{ $serviceClass->created_at->format('d/m/Y H:i') }}</p>
                </div>
                <div class="flex gap-2">
                    <a href="{{ route('classes.edit', $serviceClass) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded">
                        Edit
                    </a>
    ...
    <x-card padding="false">
        <x-slot name="title">Tarif Kelas Ini ({{ $serviceClass->tarifs->count() }})</x-slot>
        <x-table :columns="['Jenis', 'Service', 'Provider', 'Tarif', 'Periode', 'Status']" empty="Belum ada tarif untuk kelas ini." class="border-0 rounded-none shadow-none">
            @foreach($serviceClass->tarifs as $tarif)
