...
<div class="w-full mx-auto">
    <x-card padding="false">
        <x-slot name="title">Kelola Service</x-slot>
        <x-slot name="subtitle">Master data service untuk management tarif</x-slot>
        <x-slot name="actions">
            <a href="{{ route('services.create') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold text-white rounded-md bg-sp-primary hover:bg-sp-primary-dark transition-colors">
        ...
        <div class="px-4 py-3 border-b border-gray-100 bg-white">
            <form method="GET" class="flex flex-col lg:flex-row gap-3 items-end">
                <div class="flex-1">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="search" name="search" value="{{ request('search') }}"
                               placeholder="Cari kode, nama, atau deskripsi service..."
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('services.index') }}" class="inline-flex items-center px-3 py-1.5 text-sm font-semibold text-gray-600 border border-gray-300 rounded-md bg-white hover:bg-gray-50 transition-colors">
        ...
        <x-table :columns="['Kode', 'Nama', 'Deskripsi', 'Status', 'Tanggal Dibuat', 'Aksi']" :pagination="$services" empty="Tidak ada data service." class="border-0 rounded-none shadow-none">
            @foreach($services as $service)
            <tr class="hover:bg-gray-50 transition-colors">
                <td class="px-4 py-3 whitespace-nowrap">
                    <span class="inline-flex px-2 py-1 text-xs font-semibold font-mono rounded bg-gray-100 text-gray-800">{{ $service->code }}</span>
                </td>
                <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{{ $service->name }}</td>
                <td class="px-4 py-3">
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{{ $service->created_at->format('d/m/Y H:i') }}</td>
                <td class="px-4 py-3 whitespace-nowrap">
                    <x-actions>
                        <x-actions-item href="{{ route('services.show', $service) }}" icon="bi-eye" label="Detail" />
                        <x-actions-item href="{{ route('services.edit', $service) }}" icon="bi-pencil" label="Edit" />
                        <x-actions-form
                            action="{{ route('services.destroy', $service) }}"
                            method="DELETE"
                            icon="bi-trash"
                            label="Hapus"
                            confirm="Yakin ingin menghapus service ini?"
                        />
                    </x-actions>
                </td>
            </tr>
