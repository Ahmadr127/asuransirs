...
<div class="space-y-6">
            ...
            <div class="flex justify-between items-start">
                <div>
                    <div class="flex items-center gap-3 mb-2">
                        <span class="inline-flex px-2 py-1 text-xs font-mono bg-gray-200 text-gray-700 rounded">{{ $service->code }}</span>
                    ...
                    @endif
                    <p class="text-sm text-gray-500 mt-1">Dibuat {{ $service->created_at->format('d/m/Y H:i') }}</p>
                </div>
                <div class="flex gap-2">
                    <a href="{{ route('services.edit', $service) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded">
                        Edit
                    </a>
    ...
    <x-card padding="false">
        <x-slot name="title">Tarif Service Ini ({{ $service->tarifs->count() }})</x-slot>
        <x-table :columns="['Jenis', 'Provider', 'Kelas', 'Tarif', 'Periode', 'Status']" empty="Belum ada tarif untuk service ini." class="border-0 rounded-none shadow-none">
            ...
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">{{ $tarif->provider->code ?? '-' }} — {{ $tarif->provider->name ?? '-' }}</td>
