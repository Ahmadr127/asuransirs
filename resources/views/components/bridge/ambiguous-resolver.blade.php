        ...
        <div class="flex-1">
            ...
            <select name="candidate" class="w-full px-3 py-1.5 text-sm border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-sp-primary/20 focus:border-sp-primary transition-colors" required>
                @foreach($group['candidates'] as $candidate)
                    <option value="{{ $candidate['service_code'] }}|{{ $candidate['class_code'] }}">{{ $candidate['service_code'] }} | {{ $candidate['service_name'] ?? $candidate['service_code'] }} | {{ $candidate['class_code'] }} | {{ $candidate['class_name'] ?? $candidate['class_code'] }}</option>
                @endforeach
            </select>
        </div>
